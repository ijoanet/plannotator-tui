...
pub(crate) struct ThemeConfig {
    ...
    /// Syntax highlighting inside a code block: one token per scope family, matching the nine pi
    /// defines, so highlighted code reads the same here as in the agent that wrote it.
    pub(crate) syntax_comment: String,
    pub(crate) syntax_keyword: String,
    pub(crate) syntax_function: String,
    pub(crate) syntax_variable: String,
    pub(crate) syntax_string: String,
    pub(crate) syntax_number: String,
    pub(crate) syntax_type: String,
    pub(crate) syntax_operator: String,
    pub(crate) syntax_punctuation: String,
...
impl Default for Theme {
    ...
    fn default() -> Self {
        Self {
            ...
            // The families pi colours, in ANSI so a terminal's own palette still applies:
            // keywords magenta, functions and types blue, variables red, strings green, numbers
            // yellow, comments dim, and operators and punctuation left as body text.
            syntax_comment: Color::DarkGray,
            syntax_keyword: Color::Magenta,
            syntax_function: Color::Blue,
            syntax_variable: Color::Red,
...
impl Theme {
    ...
    pub(crate) fn resolve(config: &ThemeConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            syntax_comment: color(&config.syntax_comment, "syntax_comment", d.syntax_comment)?,
            syntax_keyword: color(&config.syntax_keyword, "syntax_keyword", d.syntax_keyword)?,
            syntax_function: color(&config.syntax_function, "syntax_function", d.syntax_function)?,
            syntax_variable: color(&config.syntax_variable, "syntax_variable", d.syntax_variable)?,
            syntax_string: color(&config.syntax_string, "syntax_string", d.syntax_string)?,
            syntax_number: color(&config.syntax_number, "syntax_number", d.syntax_number)?,
            syntax_type: color(&config.syntax_type, "syntax_type", d.syntax_type)?,
            syntax_operator: color(&config.syntax_operator, "syntax_operator", d.syntax_operator)?,
            syntax_punctuation: color(
                &config.syntax_punctuation,
                "syntax_punctuation",
                d.syntax_punctuation,
            )?,
